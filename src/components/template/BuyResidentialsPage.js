...
export default function BuyResidentialsPage({data}) {
  return (
    <div className='w-full max-w-5xl mx-auto flex flex-col sm:flex-row gap-4 sm:gap-6 md:gap-8 lg:gap-10 px-2 sm:px-3 md:px-4'>
        ...
        <div className='flex-1 w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 md:gap-5 mt-4 sm:mt-6 md:mt-8 lg:mt-10 content-start'>
            
            {data.length === 0 ? (
                <p className="col-span-full text-center text-gray-500 py-8 text-sm sm:text-base">
                    There is no ad to see
                </p>
            ) : (
                data.map((profile) => (
